...
class ArretDouanier extends Table
{
	
    protected static $table = 'Tb_ArretsDouanier';
    ...
    {
        if (isset($id)) {
            $sql = 'UPDATE [dbo].[' . self::getTable() . '] SET [dateEntree] = ?,[arretInfo] = ?,[arretDouanier] = ?,[diffCaisse] = ?,[diffDouanier] = ?,[observationChef] = ?,[idUser] = ? ,[versements] = ? ,[observationVersements] = ? ,[bordereauVersement] = ? ,[MontantVerse] = ? ,[idAgence] = ? WHERE [idArretsDouanier] = ?';
	...
	public static function miseJourCpta($observation,$id)
		{
			$sql = 'UPDATE [dbo].[' . self::getTable() . '] SET [observationCpta] = ? WHERE [idArretsDouanier] = ?';
	...
	public static function miseJourGestion($observation,$id)
		{
			$sql = 'UPDATE [dbo].[' . self::getTable() . '] SET [observationGestion] = ? WHERE [idArretsDouanier] = ?';
			$param = array(htmlentities($observation),htmlentities($id));
			return self::query($sql, $param);
