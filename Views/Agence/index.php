...
// Filtrer les agences par recherche si nécessaire
$agencesFiltrees = array();
if (!empty($agences) && is_array($agences)) {
    if (!empty($search)) {
        foreach ($agences as $agence) {
            if (isset($agence['designation']) && stripos($agence['designation'], $search) !== false) {
                $agencesFiltrees[] = $agence;
            }
        }
    } else {
        $agencesFiltrees = $agences;
    }
}

// PAGINATION MANUELLE
$itemsPerPage = 10;
$totalAgences = count($agencesFiltrees);
$totalPages = ceil($totalAgences / $itemsPerPage);

if ($page < 1) $page = 1;
if ($totalPages > 0 && $page > $totalPages) $page = $totalPages;

$startIndex = ($page - 1) * $itemsPerPage;
$endIndex = min($startIndex + $itemsPerPage, $totalAgences);
$agencesPagines = array_slice($agencesFiltrees, $startIndex, $itemsPerPage);
?>
...
.agences-container { max-width:100%; margin:0 auto; font-family:'Inter',sans-serif; }
...
.add-agence-btn { background:linear-gradient(135deg, var(--primary), var(--primary-dark)); border:none; color:white; }
.add-agence-btn:hover { color:white; }
...
<div class="agences-container">
    ...
    <div class="modern-header-banner">
        <h1 class="banner-title"><i class="fa fa-building"></i> GESTION DES AGENCES</h1>
        <p class="banner-subtitle">Administration et gestion des agences</p>
        
        <div class="stats-grid">
            <div class="stat-item">
                <i class="fa fa-line-chart" style="color:var(--primary);"></i>
                <div>
                    <div class="stat-value"><?php echo $totalAgences; ?></div>
    ...
    <div class="actions-section">
        <div class="actions-grid">
            <a href="#" class="modern-action-btn add-agence-btn AddAgenceCaisse">
                <i class="fa fa-plus btn-icon"></i>
                <span class="btn-text">Ajouter</span>
                <span class="btn-subtext">Nouvelle agence</span>
            </a>
        </div>
    </div>
    ...
    <div style="margin-bottom:20px;">
        <div style="display:flex; flex-wrap:wrap; gap:15px; align-items:flex-end;">
            <div style="flex:1; min-width:300px;">
                <label class="modern-form-label"><i class="fa fa-search"></i> Rechercher une agence</label>
                <div class="search-wrapper">
                    <div class="search-box">
                        <span class="search-icon"><i class="fa fa-search" style="color:#95a5a6;"></i></span>
                        <input type="text" id="search-agences" class="search-input" 
    ...
    <div class="modern-table-container">
        <div class="table-header-modern">
            <div class="table-title">
                ...
                Liste des agences
                <?php if (!empty($search)): ?>
                    (<?php echo $totalAgences; ?> résultat<?php echo $totalAgences > 1 ? 's' : ''; ?>)
                <?php else: ?>
                    (<?php echo $totalAgences; ?>)
                <?php endif; ?>
            </div>
            ...
            <table id="dynamic-table" class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Champ 1</th>
                        <th class="text-center">Champ 2</th>
                        <th class="text-center">Champ 3</th>
                        <th class="text-center" width="100">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($agencesPagines)): ?>
                        <?php for($i = 0; $i < count($agencesPagines); $i++): 
                            $agence = $agencesPagines[$i];
                            $designation = isset($agence['designation']) ? htmlspecialchars($agence['designation']) : '';
                            $id = isset($agence['id']) ? $agence['id'] : 0;
                        ?>
                        <tr>
                            <td>
                                <span class="agence-badge">
                            ...
                            <td class="text-center">
                                <div class="action-icons">
                                    <!-- Modifier -->
                                    <a class="modifierAgenceCaisse action-link green" href="#" 
                                       data-url="<?php echo App::url('ajax.agence.updateAgenceCaisse'); ?>" 
                                       data-id="<?php echo $id; ?>" 
                                       title="Modifier l'agence">
                                        <i class="fa fa-pencil"></i>
                                    </a>
                                    
                                    <!-- Supprimer (commenté car pas dans l'original mais pourrait être ajouté)
                                    <a class="deleteAgence action-link red" href="#" 
                                       data-id="<?php echo $id; ?>" 
                        ...
                        <td colspan="7" class="text-center">
                            <div class="empty-state">
                                ...
                                <p>
                                    ...
                                    <?php else: ?>
                                        Commencez par ajouter une nouvelle agence
                                    <?php endif; ?>
                                </p>
                            </div>
                        </td>
        ...
        <div style="display:flex; flex-wrap:wrap; justify-content:space-between; align-items:center; margin-top:20px;">
            <div class="pagination-info">
                Affichage de <?php echo ($totalAgences > 0 ? $startIndex + 1 : 0); ?> à <?php echo $endIndex; ?> sur <?php echo $totalAgences; ?> entrées
            </div>
            <div class="pagination-modern">
                <?php
                if ($page > 1) {
                    $prevUrl = '?p=agences&page=' . ($page - 1);
...
<!-- MODAL AJOUT AGENCE -->
<div id="modalAddAgenceCaisse" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modern-modal-header">
                <h5 class="modal-title"><i class="fa fa-plus-circle"></i> Nouvelle Agence</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body modern-modal-body">
                <form action="<?php echo App::url('ajax.agence.addAgenceCaisse'); ?>" method="POST" id="form-AddAgenceCaisse">
                    ...
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-tag"></i> Désignation <span class="required">*</span></label>
                        <input type="text" class="modern-form-control" id="designationCaisse1" name="titre" placeholder="Nom de l'agence" required>
                    </div>
                    ...
                    <div class="modern-loader loaderRegisterAg" style="display:none;">
...
<div id="modal-UpdateAgenceCaisse" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modern-modal-header">
                <h5 class="modal-title"><i class="fa fa-edit"></i> Modifier l'agence</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body modern-modal-body">
                <div id="loaderAg" class="modern-loader" style="display:none;">
                    <i class="fa fa-spinner fa-spin"></i>
                    <p>Chargement...</p>
                </div>
                <div id="chargerAg"></div>
            </div>
...
<!-- MODAL SUPPRESSION AGENCE (commenté car pas dans l'original mais peut être activé si besoin)
<div id="modal-DeleteAgence" class="modal fade">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header modern-modal-header">
                <h5 class="modal-title"><i class="fa fa-trash"></i> Supprimer l'agence</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body modern-modal-body">
                <form action="<?php echo App::url('ajax.agence.deleteAgence'); ?>" id="form-DeleteAgence">
                    <input type="hidden" class="idAgence" id="delete-agence-id">
                    <input type="hidden" class="designationAgence" id="delete-agence-designation">
                    ...
                    <div class="confirm-content">
                        ...
                        <div class="confirm-message" id="delete-message">
...
function effectuerRecherche() {
    const searchInput = document.getElementById('search-agences');
    const searchTerm = searchInput ? searchInput.value.trim() : '';
    
    let url = '?p=agences&page=1';
    if (searchTerm !== '') {
...
function effacerRecherche() {
    window.location.href = '?p=agences&page=1';
}
...
function ouvrirModalAjout() {
    const form = document.getElementById('form-AddAgenceCaisse');
    if (form) {
        form.reset();
    }
    
    if (typeof jQuery !== 'undefined' && jQuery.fn.modal) {
        jQuery('#modalAddAgenceCaisse').modal('show');
    }
}

function validerFormulaire() {
    const designation = $('#designationCaisse1').val();
    
    if (!designation || designation.trim() === '') {
        notif('Le champ Désignation est obligatoire', 'warning');
        return false;
    }
...
// MODIFICATION AGENCE
// ============================================
function ouvrirModalModifier(agenceId, url) {
    $('#loaderAg').show();
    $('#chargerAg').hide().empty();
    ...
    $.ajax({
        ...
        success: function(response) {
            $('#loaderAg').hide();
            if (response.content) {
                $('#chargerAg').html(response.content).show();
            } else {
                $('#chargerAg').html('<div class="alert alert-danger">Erreur de chargement</div>').show();
            }
        },
        error: function() {
            $('#loaderAg').hide();
            notif('Erreur de chargement', 'error');
            $('#modal-UpdateAgenceCaisse').modal('hide');
        }
    });
    
    $('#modal-UpdateAgenceCaisse').modal('show');
}

// ============================================
// SUPPRESSION AGENCE (commenté car pas dans l'original)
/*
function ouvrirModalSuppression(id, designation) {
    $('#delete-agence-id').val(id);
    $('#delete-agence-designation').val(designation);
    $('#delete-message').html('Voulez-vous vraiment supprimer l\'agence <strong>"' + escapeHtml(designation) + '"</strong> ?');
    $('#modal-DeleteAgence').modal('show');
}
*/
...
$(document).ready(function() {
    console.log('🚀 Page gestion des agences initialisée');
    ...
    // Bouton Ajouter
    $('.AddAgenceCaisse').click(function(e) {
        e.preventDefault();
        ouvrirModalAjout();
    });
    
    // Formulaire ajout
    $('#form-AddAgenceCaisse').submit(function(e) {
        e.preventDefault();
        
        if (!validerFormulaire()) return;
        
        var formData = $(this).serialize();
        var loader = $('.loaderRegisterAg');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    $('#modalAddAgenceCaisse').modal('hide');
                    notif('Agence ajoutée avec succès', 'success');
    ...
    // Bouton Modifier
    $('.modifierAgenceCaisse').click(function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        var url = $(this).data('url');
        ouvrirModalModifier(id, url);
    });
    ...
    $('#search-agences').keypress(function(e) {
        if (e.which === 13) {
            effectuerRecherche();
        }
    });
    ...
    // Suppression (commenté)
    /*
    $('.deleteAgence').click(function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        var designation = $(this).data('designation');
        ouvrirModalSuppression(id, designation);
    });
    
    $('#form-DeleteAgence').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    $('#modal-DeleteAgence').modal('hide');
                    notif('Agence supprimée avec succès', 'success');
                    
                    setTimeout(function() {
                        let url = '?p=agences&page=' + currentPage;
                        if (currentSearch !== '') {
                            url += '&search=' + encodeURIComponent(currentSearch);
                        }
                        window.location.href = url;
