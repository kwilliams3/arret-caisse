...
// Filtrer les arrêts de caisse par recherche si nécessaire
$arretsCaissesFiltres = array();
if (!empty($arretsCaissesld) && is_array($arretsCaissesld)) {
    if (!empty($search)) {
        foreach ($arretsCaissesld as $arret) {
            $found = false;
            $searchFields = ['agence'];
            ...
            $montantFields = ['arretCashCaisse', 'arretComplementCaisse', 'arretOrangeCaisse', 
                              'arretMtnCaisse', 'versements', 'MontantVerse', 'totalCaisse'];
            ...
            if ($found) {
                $arretsCaissesFiltres[] = $arret;
            }
        }
    } else {
        $arretsCaissesFiltres = $arretsCaissesld;
    }
}

// PAGINATION MANUELLE
$itemsPerPage = 10;
$totalArrets = count($arretsCaissesFiltres);
...
$arretsPagines = array_slice($arretsCaissesFiltres, $startIndex, $itemsPerPage);

// Récupérer l'agence de l'utilisateur si c'est une caissière LD
$userAgence = '';
if (in_array($user['privilege'], ['CaissiereLD'])) {
    try {
...
$privilegesAjout = ['Administration', 'CaissiereLD', 'SuperAdministration'];
...
.montant-cell { font-weight:600; color:var(--dark); }
...
.download-link { display:inline-flex; align-items:center; justify-content:center; text-decoration:none; width:32px; height:32px; border-radius:4px; background:var(--success-light); color:var(--success); transition:all 0.2s ease; }
.download-link:hover { transform:translateY(-2px); box-shadow:var(--shadow); background:var(--success); color:white; }
...
<div class="arrets-container">
    ...
    <div class="modern-header-banner">
        <h1 class="banner-title"><i class="fa fa-cash-register"></i> ARRÊTS DE CAISSE LD</h1>
        <p class="banner-subtitle">
            Gestion des arrêts de caisse • Suivi des opérations
        ...
        <div class="legend-grid">
            <div class="legend-item"><i class="fa fa-money" style="color:var(--success);"></i><span>Cash/Complément</span></div>
            <div class="legend-item"><i class="fa fa-mobile" style="color:var(--warning);"></i><span>Orange/MTN</span></div>
            <div class="legend-item"><i class="fa fa-exchange" style="color:var(--info);"></i><span>Versement</span></div>
            <div class="legend-item"><i class="fa fa-download" style="color:#8e44ad;"></i><span>Téléchargement</span></div>
        </div>
    </div>
    ...
    <div class="actions-section">
        <div class="actions-grid">
            <?php if($allowedToAdd) : ?>
            <a href="#" class="modern-action-btn add-arret-btn AddArretCaisseLD">
    ...
    <div style="margin-bottom:20px;">
        <div style="display:flex; flex-wrap:wrap; gap:15px; align-items:flex-end;">
            <div style="flex:1; min-width:300px;">
                ...
                <small class="text-muted">Recherche par agence, montant</small>
            </div>
        </div>
    </div>
    ...
    <div class="modern-table-container">
            ...
            <table id="dynamic-table" class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Cash / Complément</th>
                        <th class="text-center">Orange / MTN</th>
                        <th class="text-center">Versement / Montant</th>
                ...
                <tbody>
                    <?php if (!empty($arretsPagines)): ?>
                        <?php for($i = 0; $i < count($arretsPagines); $i++): 
                            ...
                            $arretCash = isset($arret['arretCashCaisse']) ? number_format($arret['arretCashCaisse'], 0, ',', ' ') : '0';
                            $arretComplement = isset($arret['arretComplementCaisse']) ? number_format($arret['arretComplementCaisse'], 0, ',', ' ') : '0';
                            $arretOrange = isset($arret['arretOrangeCaisse']) ? number_format($arret['arretOrangeCaisse'], 0, ',', ' ') : '0';
                            $arretMtn = isset($arret['arretMtnCaisse']) ? number_format($arret['arretMtnCaisse'], 0, ',', ' ') : '0';
                            ...
                            $bordereauLink = '';
                            $bordereauFileName = '';
                            if (isset($arret['bordereauVersement'])) {
                                $bordereau1 = explode("!", $arret['bordereauVersement']);
                                $bordereauFileName = $bordereau1[0];
                                $bordereau = "http://192.168.0.13:8088/ArretsCaisses/DocumentsBordereauLD/".$bordereauFileName;
                            }
                        ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <div><span class="montant-cell"><?php echo $arretCash; ?></span></div>
                                <div style="font-size:11px; color:var(--gray);">Complément: <?php echo $arretComplement; ?></div>
                            </td>
                            <td class="text-center">
                                <div><span class="montant-cell"><?php echo $arretOrange; ?></span></div>
                                <div style="font-size:11px; color:var(--gray);">MTN: <?php echo $arretMtn; ?></div>
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center">
                                <div class="action-icons">
                                    <a class="detailsArretsLD action-link purple" href="#" data-url="<?php echo App::url('ajax.arretsCaissesLD.detailsArretsLD'); ?>" data-id="<?php echo $arret['idArretsCaissesLD']; ?>" title="Détails">
                                        <i class="fa fa-info-circle"></i>
                                    </a>
                                    
                                    <?php if(isset($bordereauFileName) && $bordereauFileName !== 'Aucun' && !empty($bordereauFileName)): ?>
                                    <a href="<?php echo $bordereau; ?>" download="<?php echo $bordereauFileName; ?>" target="_blank" class="download-link" title="Télécharger le bordereau">
                    ...
                    <tr>
                        <td colspan="7" class="text-center">
                            <div class="empty-state">
                                ...
                                <h4>Aucun arrêt de caisse trouvé</h4>
                                <p>
                                    <?php if (!empty($search)): ?>
        ...
        <div style="display:flex; flex-wrap:wrap; justify-content:space-between; align-items:center; margin-top:20px;">
            ...
            <div class="pagination-modern">
                <?php
                if ($page > 1) {
                    $prevUrl = '?p=arretsCaissesLD&page=' . ($page - 1);
                ...
                if ($startPage > 1) {
                    $firstUrl = '?p=arretsCaissesLD&page=1';
                ...
                for ($i = $startPage; $i <= $endPage; $i++) {
                    if ($i == $page) {
                        ...
                        $pageUrl = '?p=arretsCaissesLD&page=' . $i;
...
<!-- MODAL DÉTAILS ARRÊT LD -->
<div id="modal-detailsArretsLD" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modern-modal-header">
                <h5 class="modal-title"><i class="fa fa-info-circle"></i> Détails de l'arrêt de caisse</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body modern-modal-body">
                <div id="loaderArretsLD" class="modern-loader" style="display:none;">
                    <i class="fa fa-spinner fa-spin"></i>
                    <p>Chargement des détails...</p>
                </div>
                <div id="chargerArretsLD"></div>
            </div>
...
<!-- MODAL AJOUT ARRÊT CAISSE LD -->
<div id="modalAjoutArretCaisseLD" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modern-modal-header">
                <h5 class="modal-title"><i class="fa fa-plus-circle"></i> Ajouter un Arrêt de Caisse LD</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body modern-modal-body">
                <form action="<?php echo App::url('ajax.arretsCaissesLD.ajoutArretsCaisseLD'); ?>" method="POST" id="form-ajoutArretsCaisseLD" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-plus-circle"></i> Complément en caisse</label>
                        <input type="number" class="modern-form-control" id="complementCashLD" name="complementCashLD" placeholder="Complément en caisse" value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-money"></i> Arrêt Cash</label>
                        <input type="number" class="modern-form-control" id="arretCashLD" name="arretCashLD" placeholder="Arrêt Cash" required value="0">
                    </div>

                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-mobile"></i> Arrêt Orange Money</label>
                        <input type="number" class="modern-form-control" id="arretOrangeMobileLD" name="arretOrangeMobileLD" placeholder="Arrêt Orange Money" required value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-mobile"></i> Arrêt MTN Money</label>
                        <input type="number" class="modern-form-control" id="arretMtnMobileLD" name="arretMtnMobileLD" placeholder="Arrêt MTN Money" required value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-credit-card"></i> Arrêt TPE</label>
                        <input type="number" class="modern-form-control" id="arretTpeMobileLD" name="arretTpeMobileLD" placeholder="Arrêt TPE" required value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-credit-card"></i> Arrêt Carte Sorepco</label>
                        <input type="number" class="modern-form-control" id="arretCarteLD" name="arretCarteLD" placeholder="Arrêt Carte Sorepco" required value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-money"></i> Arrêt Chèque</label>
                        <input type="number" class="modern-form-control" id="arretChequeLD" name="arretChequeLD" placeholder="Arrêt Chèque" required value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-exchange"></i> Arrêt Virement</label>
                        <input type="number" class="modern-form-control" id="arretVirementLD" name="arretVirementLD" placeholder="Arrêt Virement" required value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-info-circle"></i> Arrêt Info</label>
                        <input type="number" class="modern-form-control" id="arretInfoLD" name="arretInfoLD" placeholder="Arrêt Info" required value="0">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-calculator"></i> Total Caisse</label>
                        <input type="number" class="modern-form-control" id="totalCaisse" name="totalCaisse1" placeholder="Total Caisse" readonly style="background:var(--primary-light); font-weight:700;">
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-exchange"></i> Versement Banque / Ramassage</label>
                        <select class="modern-form-control" id="versementLD" name="versementLD">
                    ...
                    <div class="form-group conditional-field" id="observationGroup" style="display:none;">
                        <label class="modern-form-label"><i class="fa fa-comment"></i> Observation Versement</label>
                        <textarea class="modern-form-control" id="observationVersLD" name="observationVersLD" placeholder="Entrer les observations versement" rows="2"></textarea>
                    </div>
                    
                    <div class="form-group conditional-field" id="montantGroup" style="display:none;">
                        <label class="modern-form-label"><i class="fa fa-money"></i> Montant versement</label>
                        <input type="number" class="modern-form-control" id="montantVerseLD" name="montantVerseLD" placeholder="Montant Versé">
                    </div>
                    
                    <div class="form-group conditional-field" id="bordereauGroup" style="display:none;">
                        <label class="modern-form-label"><i class="fa fa-file-pdf-o"></i> Bordereau Versement / Ramassage</label>
                        <input type="file" class="modern-form-control" id="bordereauVersLD" name="bordereauVersLD" accept=".pdf,.jpg,.jpeg,.png">
                    ...
                    <div class="alert alert-warning" style="background:var(--warning-light); border-left:4px solid var(--warning); padding:15px; border-radius:8px;">
                        ...
                        <strong>Note :</strong> Si vous sélectionnez "Oui" pour le versement, veuillez remplir les champs supplémentaires.
                    </div>
...
// ============================================
// CALCUL DU TOTAL CAISSE
// ============================================
function calculerTotalCaisse() {
    let complement = parseFloat($('#complementCashLD').val()) || 0;
    let arretCash = parseFloat($('#arretCashLD').val()) || 0;
    let arretOrange = parseFloat($('#arretOrangeMobileLD').val()) || 0;
    let arretMtn = parseFloat($('#arretMtnMobileLD').val()) || 0;
    let arretTpe = parseFloat($('#arretTpeMobileLD').val()) || 0;
    let arretCarte = parseFloat($('#arretCarteLD').val()) || 0;
    let arretCheque = parseFloat($('#arretChequeLD').val()) || 0;
    let arretVirement = parseFloat($('#arretVirementLD').val()) || 0;
    let arretInfo = parseFloat($('#arretInfoLD').val()) || 0;
    
    let total = complement + arretCash + arretOrange + arretMtn + arretTpe + arretCarte + arretCheque + arretVirement + arretInfo;
...
function toggleConditionalFields() {
    let versement = $('#versementLD').val();
    
    if (versement === 'Oui') {
        $('#observationGroup, #montantGroup, #bordereauGroup').slideDown();
    } else {
        $('#observationGroup, #montantGroup, #bordereauGroup').slideUp();
        $('#observationVersLD').val('');
        $('#montantVerseLD').val('');
        $('#bordereauVersLD').val('');
    }
}
...
function effectuerRecherche() {
    ...
    let url = '?p=arretsCaissesLD&page=1';
    if (searchTerm !== '') {
...
function effacerRecherche() {
    window.location.href = '?p=arretsCaissesLD&page=1';
}

// ============================================
// DÉTAILS ARRÊT
// ============================================
function ouvrirModalDetails(element) {
    var id = $(element).data('id');
    var url = $(element).data('url');
    
    $('#loaderArretsLD').show();
    $('#chargerArretsLD').hide().empty();
    $('#modal-detailsArretsLD').modal('show');
        ...
        success: function(response) {
            ...
            if (typeof response === 'string') {
                try {
                    ...
                    htmlContent = parsed.content || response;
                } catch(e) {
                ...
                htmlContent = '<div class="alert alert-danger">Erreur de chargement</div>';
            }
            
            $('#chargerArretsLD').html(htmlContent).show();
            $('#loaderArretsLD').hide();
        },
        error: function() {
            notif('Erreur de chargement des détails', 'error');
            $('#loaderArretsLD').hide();
            $('#modal-detailsArretsLD').modal('hide');
        }
    });
}

// ============================================
// AJOUT D'ARRÊT CAISSE
// ============================================
function ouvrirModalAjout() {
    console.log('📋 Ouverture modal ajout');
    
    const form = document.getElementById('form-ajoutArretsCaisseLD');
    if (form) {
        form.reset();
    }
    ...
    if (typeof jQuery !== 'undefined' && jQuery.fn.modal) {
        jQuery('#modalAjoutArretCaisseLD').modal('show');
    }
}
...
function validerFormulaire() {
    ...
    const inputs = [
        { id: 'complementCashLD', nom: 'Complément en caisse' },
        { id: 'arretCashLD', nom: 'Arrêt Cash' },
        { id: 'arretOrangeMobileLD', nom: 'Arrêt Orange Money' },
        { id: 'arretMtnMobileLD', nom: 'Arrêt MTN Money' },
        { id: 'arretTpeMobileLD', nom: 'Arrêt TPE' },
        { id: 'arretCarteLD', nom: 'Arrêt Carte' },
        { id: 'arretChequeLD', nom: 'Arrêt Chèque' },
        { id: 'arretVirementLD', nom: 'Arrêt Virement' },
        { id: 'arretInfoLD', nom: 'Arrêt Info' }
    ];
    ...
    let versement = $('#versementLD').val();
    if (versement === 'Oui') {
        let montantVerse = $('#montantVerseLD').val();
        if (!montantVerse) {
            $('#montantVerseLD').addClass('is-invalid');
...
$(document).ready(function() {
    console.log('🚀 Page arrêts de caisse LD initialisée');
    ...
    // Bouton Ajouter
    $('.AddArretCaisseLD').click(function(e) {
        e.preventDefault();
        ouvrirModalAjout();
    });
    
    // Détails
    $(document).on('click', '.detailsArretsLD', function(e) {
        e.preventDefault();
        ouvrirModalDetails(this);
    });
    
    // Calcul automatique du total
    $('#complementCashLD, #arretCashLD, #arretOrangeMobileLD, #arretMtnMobileLD, #arretTpeMobileLD, #arretCarteLD, #arretChequeLD, #arretVirementLD, #arretInfoLD').on('input', function() {
        calculerTotalCaisse();
    });
    
    // Gestion des champs conditionnels
    $('#versementLD').change(function() {
        toggleConditionalFields();
    });
    
    // Soumission du formulaire
    $('#form-ajoutArretsCaisseLD').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    $('#modalAjoutArretCaisseLD').modal('hide');
                    notif('Arrêt de caisse ajouté avec succès', 'success');
