...
if (!empty($arretsCaisses) && is_array($arretsCaisses)) {
    if (!empty($search)) {
        foreach ($arretsCaisses as $arret) {
            $found = false;
            $searchFields = ['agence', 'totalCaisse'];
            ...
            $montantFields = ['arretCashCaisse', 'arretComplementCaisse', 'arretOrangeCaisse', 'arretMtnCaisse', 
                              'arretCarteCaisse', 'totalBonPros', 'arretChequeCaisse', 'arretTpeCaisse'];
        ...
        $arretsCaissesFiltres = $arretsCaisses;
    }
}
...
// Récupérer l'agence de l'utilisateur si c'est une caissière SAGE
$userAgence = '';
if (in_array($user['privilege'], ['CaissiereSage'])) {
    try {
...
$privilegesAjout = ['Administration', 'CaissiereSage', 'SuperAdministration'];
...
<div class="arrets-sage-container">
    ...
    <div class="modern-header-banner">
        <h1 class="banner-title"><i class="fa fa-cash-register"></i> ARRÊTS DE CAISSE SAGE</h1>
        <p class="banner-subtitle">
            Gestion des arrêts de caisse SAGE • Suivi des opérations
        ...
        <div class="legend-grid">
            ...
            <div class="legend-item"><i class="fa fa-credit-card" style="color:var(--info);"></i><span>Carte/Bon Pros</span></div>
            <div class="legend-item"><i class="fa fa-money-check" style="color:#8e44ad;"></i><span>Chèque/TPE</span></div>
        </div>
    </div>
    ...
    <div class="actions-section">
        <div class="actions-grid">
            <?php if($allowedToAdd) : ?>
            <a href="#" class="modern-action-btn add-arret-sage-btn AddArretCaissesSage">
                <i class="fa fa-plus btn-icon"></i>
                <span class="btn-text">Ajouter</span>
                <span class="btn-subtext">Nouvel arrêt SAGE</span>
            </a>
            <?php endif; ?>
        </div>
    </div>
            ...
            <div style="flex:1; min-width:300px;">
                ...
                <div class="search-wrapper">
                    <div class="search-box">
                        ...
                        <input type="text" id="search-arrets-sage" class="search-input" 
                ...
                <small class="text-muted">Recherche par agence, montant, date</small>
            </div>
        </div>
    </div>
    ...
    <div class="modern-table-container">
        <div class="table-header-modern">
            <div class="table-title">
                ...
                Liste des arrêts de caisse SAGE
                <?php if (!empty($search)): ?>
            ...
            <table id="dynamic-table" class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Carte / Bon Pros</th>
                        <th class="text-center">Chèque / TPE</th>
                ...
                <tbody>
                    <?php if (!empty($arretsPagines)): ?>
                        <?php for($i = 0; $i < count($arretsPagines); $i++): 
                            ...
                            $arretCarte = isset($arret['arretCarteCaisse']) ? number_format($arret['arretCarteCaisse'], 0, ',', ' ') : '0';
                            $totalBonPros = isset($arret['totalBonPros']) ? number_format($arret['totalBonPros'], 0, ',', ' ') : '0';
                            $arretCheque = isset($arret['arretChequeCaisse']) ? number_format($arret['arretChequeCaisse'], 0, ',', ' ') : '0';
                            $arretTpe = isset($arret['arretTpeCaisse']) ? number_format($arret['arretTpeCaisse'], 0, ',', ' ') : '0';
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div><span class="montant-cell"><?php echo $arretCarte; ?></span></div>
                                <div style="font-size:11px; color:var(--gray);">Bon Pros: <?php echo $totalBonPros; ?></div>
                            </td>
                            <td class="text-center">
                                <div><span class="montant-cell"><?php echo $arretCheque; ?></span></div>
                                <div style="font-size:11px; color:var(--gray);">TPE: <?php echo $arretTpe; ?></div>
                            </td>
                            <td class="text-center">
                    ...
                    <tr>
                        <td colspan="7" class="text-center">
                            <div class="empty-state">
                                ...
                                <h4>Aucun arrêt de caisse SAGE trouvé</h4>
                                <p>
                                    <?php if (!empty($search)): ?>
        ...
        <div style="display:flex; flex-wrap:wrap; justify-content:space-between; align-items:center; margin-top:20px;">
            ...
            <div class="pagination-modern">
                <?php
                if ($page > 1) {
                    $prevUrl = '?p=arretsCaissesSage&page=' . ($page - 1);
                ...
                if ($startPage > 1) {
                    $firstUrl = '?p=arretsCaissesSage&page=1';
                ...
                for ($i = $startPage; $i <= $endPage; $i++) {
                    if ($i == $page) {
                        ...
                        $pageUrl = '?p=arretsCaissesSage&page=' . $i;
...
<!-- MODAL AJOUT ARRÊT CAISSE SAGE -->
<div id="modalAjoutArretCaisseSage" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modern-modal-header">
                <h5 class="modal-title"><i class="fa fa-plus-circle"></i> Ajouter un Arrêt de Caisse SAGE</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body modern-modal-body">
                <form action="<?php echo App::url('ajax.arretsCaisses.ajoutArretsCaisseSage'); ?>" method="POST" id="form-ajoutArretsCaisseSage">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="modern-form-control" id="complementCashSage1" name="complementCashSage" placeholder="Montant complément" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-money"></i> Arrêt Cash</label>
                        <input type="number" class="modern-form-control" id="arretCashSage1" name="arretCashSage" placeholder="Montant en espèces" required>
                    </div>

                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-mobile"></i> Arrêt Orange Money</label>
                        <input type="number" class="modern-form-control" id="arretOrangeMobileSage1" name="arretOrangeMobileSage" placeholder="Montant Orange Money" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-mobile"></i> Arrêt MTN Money</label>
                        <input type="number" class="modern-form-control" id="arretMtnMobileSage1" name="arretMtnMobileSage" placeholder="Montant MTN Money" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-credit-card"></i> Arrêt TPE</label>
                        <input type="number" class="modern-form-control" id="arretTpeMobileSage1" name="arretTpeMobileSage" placeholder="Montant TPE" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-credit-card"></i> Arrêt Carte Sorepco</label>
                        <input type="number" class="modern-form-control" id="arretCarteSage1" name="arretCarteSage" placeholder="Montant Carte Sorepco" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-money-check"></i> Arrêt Chèque</label>
                        <input type="number" class="modern-form-control" id="arretChequeSage1" name="arretChequeSage" placeholder="Montant Chèque" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-exchange"></i> Arrêt Virement / Versement Client</label>
                        <input type="number" class="modern-form-control" id="arretVirementSage1" name="arretVirementSage" placeholder="Montant Virement" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-users"></i> Total Bons Prospecteur</label>
                        <input type="number" class="modern-form-control" id="totalBonProsSage1" name="totalBonProsSage" placeholder="Total des bons prospecteurs" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-calculator"></i> Total Caisse</label>
                        <input type="number" class="modern-form-control" id="totalCaisseSage1" name="totalCaisseSage" placeholder="Total caisse" readonly style="background:var(--primary-light); font-weight:700;">
                    </div>
                    ...
                    <div class="alert alert-warning" style="background:var(--warning-light); border-left:4px solid var(--warning); padding:15px; border-radius:8px;">
                        ...
                        <strong>Note :</strong> Le total caisse est calculé automatiquement.
                    </div>
...
function calculerTotalCaisse() {
    let complementCash = parseFloat($('#complementCashSage1').val()) || 0;
    let arretCash = parseFloat($('#arretCashSage1').val()) || 0;
    let arretOrange = parseFloat($('#arretOrangeMobileSage1').val()) || 0;
    let arretMtn = parseFloat($('#arretMtnMobileSage1').val()) || 0;
    let arretTpe = parseFloat($('#arretTpeMobileSage1').val()) || 0;
    let arretCarte = parseFloat($('#arretCarteSage1').val()) || 0;
    let arretCheque = parseFloat($('#arretChequeSage1').val()) || 0;
    let arretVirement = parseFloat($('#arretVirementSage1').val()) || 0;
    let totalBonPros = parseFloat($('#totalBonProsSage1').val()) || 0;
    
    let total = complementCash + arretCash + arretOrange + arretMtn + arretTpe + arretCarte + arretCheque + arretVirement + totalBonPros;
...
function effectuerRecherche() {
    const searchInput = document.getElementById('search-arrets-sage');
    const searchTerm = searchInput.value.trim();
    
    let url = '?p=arretsCaissesSage&page=1';
    if (searchTerm !== '') {
...
function effacerRecherche() {
    window.location.href = '?p=arretsCaissesSage&page=1';
}

// ============================================
// AJOUT D'ARRÊT CAISSE SAGE
// ============================================
function ouvrirModalAjout() {
    console.log('📋 Ouverture modal ajout arrêt SAGE');
    
    const form = document.getElementById('form-ajoutArretsCaisseSage');
    if (form) {
        form.reset();
    }
    
    $('#totalCaisseSage1').val(0);
    
    if (typeof jQuery !== 'undefined' && jQuery.fn.modal) {
        jQuery('#modalAjoutArretCaisseSage').modal('show');
    }
}
...
function validerFormulaire() {
    ...
    const inputs = [
        { id: 'complementCashSage1', nom: 'Complément en caisse' },
        { id: 'arretCashSage1', nom: 'Arrêt Cash' },
        { id: 'arretOrangeMobileSage1', nom: 'Arrêt Orange Money' },
        { id: 'arretMtnMobileSage1', nom: 'Arrêt MTN Money' },
        { id: 'arretTpeMobileSage1', nom: 'Arrêt TPE' },
        { id: 'arretCarteSage1', nom: 'Arrêt Carte' },
        { id: 'arretChequeSage1', nom: 'Arrêt Chèque' },
        { id: 'arretVirementSage1', nom: 'Arrêt Virement' },
        { id: 'totalBonProsSage1', nom: 'Total Bons Prospecteur' }
    ];
...
$(document).ready(function() {
    console.log('🚀 Page arrêts de caisse SAGE initialisée');
    ...
    // Bouton Ajouter
    $('.AddArretCaissesSage').click(function(e) {
        e.preventDefault();
        ouvrirModalAjout();
    });
    
    // Calcul automatique du total
    $('#complementCashSage1, #arretCashSage1, #arretOrangeMobileSage1, #arretMtnMobileSage1, #arretTpeMobileSage1, #arretCarteSage1, #arretChequeSage1, #arretVirementSage1, #totalBonProsSage1').on('input', function() {
        calculerTotalCaisse();
    });
    
    // Soumission du formulaire
    $('#form-ajoutArretsCaisseSage').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    $('#modalAjoutArretCaisseSage').modal('hide');
                    notif('Arrêt de caisse SAGE ajouté avec succès', 'success');
    ...
    $('#search-arrets-sage').keypress(function(e) {
        if (e.which === 13) {
            effectuerRecherche();
        }
    });
