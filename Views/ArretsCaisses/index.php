...
if (!empty($arretsCaisses) && is_array($arretsCaisses)) {
    if (!empty($search)) {
        foreach ($arretsCaisses as $arret) {
            ...
            $montantFields = ['arretCashCaisse', 'totalBonPros', 'arretOrangeCaisse', 'arretMtnCaisse', 
                              'arretCarteCaisse', 'arretVirementCaisse', 'arretChequeCaisse', 'arretTpeCaisse'];
...
// Récupérer l'agence de l'utilisateur si c'est une caissière
$userAgence = '';
if (in_array($user['privilege'], ['Caissiere'])) {
    try {
...
$privilegesAjout = ['Administration', 'Caissiere', 'SuperAdministration'];
...
<div class="arrets-container">
    ...
    <div class="modern-header-banner">
        <h1 class="banner-title"><i class="fa fa-cash-register"></i> ARRÊTS DE CAISSE</h1>
        ...
        <div class="legend-grid">
            <div class="legend-item"><i class="fa fa-money" style="color:var(--success);"></i><span>Cash</span></div>
            <div class="legend-item"><i class="fa fa-mobile" style="color:var(--warning);"></i><span>Orange/MTN</span></div>
            <div class="legend-item"><i class="fa fa-credit-card" style="color:var(--info);"></i><span>Carte/Virement</span></div>
            <div class="legend-item"><i class="fa fa-money" style="color:var(--purple);"></i><span>Chèque/TPE</span></div>
        </div>
    </div>
    ...
    <div class="actions-section">
        <div class="actions-grid">
            <?php if($allowedToAdd) : ?>
            <a href="#" class="modern-action-btn add-arret-btn AddArretCaisse">
    ...
    <div class="modern-table-container">
            ...
            <table id="dynamic-table" class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Cash / Bon Pros</th>
                        <th class="text-center">Orange / MTN</th>
                        <th class="text-center">Carte / Virement</th>
                ...
                <tbody>
                    <?php if (!empty($arretsPagines)): ?>
                        <?php for($i = 0; $i < count($arretsPagines); $i++): 
                            ...
                            $arretVirement = isset($arret['arretVirementCaisse']) ? number_format($arret['arretVirementCaisse'], 0, ',', ' ') : '0';
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <div style="font-size:11px; color:var(--gray);">Virement: <?php echo $arretVirement; ?></div>
                            </td>
                            <td class="text-center">
        ...
        <div style="display:flex; flex-wrap:wrap; justify-content:space-between; align-items:center; margin-top:20px;">
            ...
            <div class="pagination-modern">
                <?php
                if ($page > 1) {
                    $prevUrl = '?p=arretsCaisses&page=' . ($page - 1);
                ...
                if ($startPage > 1) {
                    $firstUrl = '?p=arretsCaisses&page=1';
                ...
                for ($i = $startPage; $i <= $endPage; $i++) {
                    if ($i == $page) {
                        ...
                        $pageUrl = '?p=arretsCaisses&page=' . $i;
...
<!-- MODAL AJOUT ARRÊT CAISSE -->
<div id="modalAjoutArretCaisse" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modern-modal-header">
                <h5 class="modal-title"><i class="fa fa-plus-circle"></i> Ajouter un Arrêt de Caisse</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body modern-modal-body">
                <form action="<?php echo App::url('ajax.arretsCaisses.ajoutArretsCaisse'); ?>" method="POST" id="form-ajoutArretsCaisse">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="modern-form-control" id="arretCash1" name="arretCash" placeholder="Montant en espèces" required>
                    </div>

                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-mobile"></i> Arrêt Orange Money</label>
                        <input type="number" class="modern-form-control" id="arretOrangeMobile1" name="arretOrangeMobile" placeholder="Montant Orange Money" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-mobile"></i> Arrêt MTN Money</label>
                        <input type="number" class="modern-form-control" id="arretMtnMobile1" name="arretMtnMobile" placeholder="Montant MTN Money" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-credit-card"></i> Arrêt TPE</label>
                        <input type="number" class="modern-form-control" id="arretTpeMobile1" name="arretTpeMobile" placeholder="Montant TPE" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-credit-card"></i> Arrêt Carte Sorepco</label>
                        <input type="number" class="modern-form-control" id="arretCarte1" name="arretCarte" placeholder="Montant Carte Sorepco" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-money"></i> Arrêt Chèque</label>
                        <input type="number" class="modern-form-control" id="arretCheque1" name="arretCheque" placeholder="Montant Chèque" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-exchange"></i> Arrêt Virement / Versement Client</label>
                        <input type="number" class="modern-form-control" id="arretVirement1" name="arretVirement" placeholder="Montant Virement" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="modern-form-label"><i class="fa fa-users"></i> Total Bon Prospecteur</label>
                        <input type="number" class="modern-form-control" id="totalBonPros" name="totalBonPros" placeholder="Total des bons prospecteurs" required>
                    </div>
...
function calculerTotalCaisse() {
    let arretCash = parseFloat($('#arretCash1').val()) || 0;
    let arretOrange = parseFloat($('#arretOrangeMobile1').val()) || 0;
    let arretMtn = parseFloat($('#arretMtnMobile1').val()) || 0;
    let arretTpe = parseFloat($('#arretTpeMobile1').val()) || 0;
    let arretCarte = parseFloat($('#arretCarte1').val()) || 0;
    let arretCheque = parseFloat($('#arretCheque1').val()) || 0;
    let arretVirement = parseFloat($('#arretVirement1').val()) || 0;
    let totalBonPros = parseFloat($('#totalBonPros').val()) || 0;
    
    let total = arretCash + arretOrange + arretMtn + arretTpe + arretCarte + arretCheque + arretVirement + totalBonPros;
...
function effectuerRecherche() {
    ...
    let url = '?p=arretsCaisses&page=1';
    if (searchTerm !== '') {
...
function effacerRecherche() {
    window.location.href = '?p=arretsCaisses&page=1';
}
...
function ouvrirModalAjout() {
    ...
    const form = document.getElementById('form-ajoutArretsCaisse');
    if (form) {
        form.reset();
    }
    ...
    if (typeof jQuery !== 'undefined' && jQuery.fn.modal) {
        jQuery('#modalAjoutArretCaisse').modal('show');
    }
}
...
function validerFormulaire() {
    ...
    const inputs = [
        { id: 'arretCash1', nom: 'Arrêt Cash' },
        { id: 'arretOrangeMobile1', nom: 'Arrêt Orange Money' },
        { id: 'arretMtnMobile1', nom: 'Arrêt MTN Money' },
        { id: 'arretTpeMobile1', nom: 'Arrêt TPE' },
        { id: 'arretCarte1', nom: 'Arrêt Carte' },
        { id: 'arretCheque1', nom: 'Arrêt Chèque' },
        { id: 'arretVirement1', nom: 'Arrêt Virement' },
        { id: 'totalBonPros', nom: 'Total Bon Prospecteur' }
    ];
...
$(document).ready(function() {
    console.log('🚀 Page arrêts de caisse initialisée');
    ...
    // Bouton Ajouter
    $('.AddArretCaisse').click(function(e) {
        e.preventDefault();
        ouvrirModalAjout();
    });
    
    // Calcul automatique du total
    $('#arretCash1, #arretOrangeMobile1, #arretMtnMobile1, #arretTpeMobile1, #arretCarte1, #arretCheque1, #arretVirement1, #totalBonPros').on('input', function() {
        calculerTotalCaisse();
    });
    
    // Soumission du formulaire
    $('#form-ajoutArretsCaisse').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    $('#modalAjoutArretCaisse').modal('hide');
